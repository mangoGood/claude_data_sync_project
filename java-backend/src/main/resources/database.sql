-- ============================================================
-- Sync Task Backend 数据库初始化脚本
-- 基于 JPA 实体类自动对齐，包含所有业务表结构
-- ============================================================
...
CREATE TABLE IF NOT EXISTS workflows (
    ...
    migration_mode ENUM('full', 'fullAndIncre', 'subscribe') DEFAULT 'full' COMMENT '迁移模式',
...
ALTER TABLE workflows ADD COLUMN IF NOT EXISTS dr_switch_start_time TIMESTAMP NULL COMMENT '灾备倒换开始时间';
ALTER TABLE workflows ADD COLUMN IF NOT EXISTS kafka_bootstrap_servers VARCHAR(500) COMMENT 'Kafka连接地址';
ALTER TABLE workflows ADD COLUMN IF NOT EXISTS kafka_topic_prefix VARCHAR(100) DEFAULT 'cdc' COMMENT 'Kafka主题前缀';
ALTER TABLE workflows ADD COLUMN IF NOT EXISTS kafka_topic_strategy VARCHAR(20) DEFAULT 'TABLE' COMMENT 'Kafka主题策略';
...
-- 迁移：更新状态枚举（兼容 RECEIVED 状态）
ALTER TABLE workflows MODIFY COLUMN status ENUM('CONFIGURING', 'PENDING', 'RECEIVED', 'STARTING', 'FULL_MIGRATING',
                 'FULL_COMPLETED', 'INCREMENT_RUNNING', 'SUBSCRIBE_RUNNING',
                 'SWITCHING', 'COMPLETED', 'FAILED', 'PAUSED') DEFAULT 'CONFIGURING' COMMENT '任务状态';
...
CREATE TABLE IF NOT EXISTS audit_logs (
    ...
    action ENUM('CREATE_TASK','UPDATE_CONFIG','LAUNCH_TASK','PAUSE_TASK','RESUME_TASK',
                'STOP_TASK','DELETE_TASK','RETRY_TASK','FAILOVER_TASK','LOGIN','LOGOUT') NOT NULL COMMENT '操作类型',
...
CREATE TABLE IF NOT EXISTS slow_sql_records (
    ...
    threshold_ms BIGINT COMMENT '慢SQL阈值(毫秒)',
...
CREATE TABLE IF NOT EXISTS data_validations (
    ...
    source_db_name VARCHAR(200) COMMENT '源库名',
    target_db_name VARCHAR(200) COMMENT '目标库名',
    source_count BIGINT COMMENT '源行数',
    target_count BIGINT COMMENT '目标行数',
    mismatched_count BIGINT COMMENT '不一致行数',
    ...
    repair_status VARCHAR(20) COMMENT '修复状态',
    error_message TEXT COMMENT '错误信息',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    completed_at TIMESTAMP NULL COMMENT '完成时间',
...
CREATE TABLE IF NOT EXISTS alert_rules (
    ...
    email_recipients VARCHAR(500) COMMENT '邮件收件人',
    enabled TINYINT(1) DEFAULT 1 COMMENT '是否启用',
...
CREATE TABLE IF NOT EXISTS task_schedules (
    ...
    schedule_name VARCHAR(200) COMMENT '调度名称',
    schedule_type VARCHAR(20) DEFAULT 'FULL_SYNC' COMMENT '调度类型: FULL_SYNC/VALIDATION',
    enabled TINYINT(1) DEFAULT 1 COMMENT '是否启用',
    last_triggered_at TIMESTAMP NULL COMMENT '最后触发时间',
    next_trigger_at TIMESTAMP NULL COMMENT '下次触发时间',
    trigger_count INT DEFAULT 0 COMMENT '触发次数',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
