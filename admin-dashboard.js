...
// 以 test_scripts/e2e_smoke.py 作回归网分批推进。
        ...
        function handleStatusUpdate(data) {
            ...
            if (drRow) {
                let drStatusInfo = drStatusMap[data.status] || drStatusMap['PENDING'];
        ...
        function createTableRow(workflow) {
            ...
            row.innerHTML = `
                <div class="table-cell col-checkbox">
                    <input type="checkbox" class="batch-cb" data-id="${workflow.id}" ${batchSelected.has(workflow.id) ? 'checked' : ''}
        ...
        function refreshCurrentTaskList() {
            ...
            if (drPageEl && drPageEl.style.display !== 'none') {
                fetchDrTasks(typeof drCurrentPage !== 'undefined' ? drCurrentPage : 1);
            } else {
                fetchWorkflows();
            }
        }
                ...
                if (result.success) {
                    ...
                    if (dlSkipBtn) {
                        ...
                        if (task.status === 'FAILED' && seqMatch) {
                            _dlFailedSeqno = parseInt(seqMatch[1], 10);
        ...
        window.advOnSchedTypeChange = function() {
        ...
        window.advCreateSchedule = async function() {
        ...
        window.advToggleSchedule = async function(id, enable) {
        ...
        window.advDeleteSchedule = async function(id) {
        ...
        window.advCreateRetryPolicy = async function() {
        ...
        window.advDeleteRetryPolicy = async function(id) {
        ...
        window.advCreateDependency = async function() {
        ...
        window.advDeleteDependency = async function(id) {
        ...
        window.advOnAlertMetricChange = function() {
        ...
        window.advOnAlertChannelChange = function() {
        ...
        window.advCreateAlertRule = async function() {
        ...
        window.advDeleteAlertRule = async function(id) {
        ...
        window.advLoadAlertEvents = async function(page) {
        ...
        window.advAlertEventPage = function(delta) {
        ...
        window.advLoadSlowSqlPage = async function(page) {
        ...
        window.advSlowSqlPage = function(delta) {
        ...
        window.advDiagnoseTask = async function() {
            if (!_advDetailTaskId) return;
            ...
            const d = await advFetchJson(`${ADV_API}/diagnose/${_advDetailTaskId}`, { method: 'POST', headers: getAuthHeaders() });
            if (!d.success) {
        ...
        window.advShowConfigVersions = async function() {
            if (!_advDetailTaskId) return;
            ...
            modal.classList.add('show');
            const d = await advFetchJson(`${ADV_API}/config-versions/${_advDetailTaskId}`);
            const list = d.success ? (d.data || []) : [];
            if (list.length === 0) {
        ...
        window.advRollbackConfig = async function(versionNumber) {
            if (!_advDetailTaskId) return;
            if (!confirm(`确定回滚配置到版本 v${versionNumber}？`)) return;
            const d = await advFetchJson(`${ADV_API}/config-versions/${_advDetailTaskId}/rollback/${versionNumber}`,
                { method: 'POST', headers: getAuthHeaders() });
            if (d.success) {
        ...
        window.advCloneTask = async function() {
            if (!_advDetailTaskId) return;
            const newName = prompt('请输入克隆后的任务名称：');
            if (newName === null) return;
            if (!newName.trim()) { showNotification('任务名称不能为空', 'error'); return; }
            const d = await advFetchJson(`${ADV_API}/clone/${_advDetailTaskId}`, {
                method: 'POST', headers: getAuthHeaders(),
        ...
        let _dlFailedSeqno = null;

        window.dlSkipEventAndRetry = async function() {
            if (!_advDetailTaskId || _dlFailedSeqno === null) return;
            if (!confirm(`确定跳过失败事件 seqno=${_dlFailedSeqno} 并恢复任务？\n\n该事件将【不会】被应用到目标库，仅记入死信记录；源和目标可能因此产生差异，需人工核对补偿。`)) return;
            try {
                const resp = await fetchWithAuth(`${API_BASE_URL}/workflows/${_advDetailTaskId}/skip-event`, {
                    method: 'POST', headers: getAuthHeaders(),
                    body: JSON.stringify({ seqno: _dlFailedSeqno })
                });
        ...
        window.dlShowDeadletter = async function() {
            if (!_advDetailTaskId) return;
            ...
            let d;
            try {
                const resp = await fetchWithAuth(`${API_BASE_URL}/workflows/${_advDetailTaskId}/deadletter`);
                d = await resp.json();
            } catch (e) {
        ...
        window.cpShowCheckpoint = function() {
            if (!_advDetailTaskId) return;
        ...
        window.cpCloseCheckpoint = function() {
        ...
        window.cpLoadCheckpoint = async function() {
            if (!_advDetailTaskId) return;
            const body = document.getElementById('cpCheckpointBody');
            let d;
            try {
                const resp = await fetchWithAuth(`${API_BASE_URL}/workflows/${_advDetailTaskId}/checkpoint`);
                d = await resp.json();
            } catch (e) {
        ...
        window.advToggleBatchSelect = function(id, checked) {
            if (checked) batchSelected.add(id); else batchSelected.delete(id);
            advUpdateBatchToolbar();
        }

        window.advToggleBatchAll = function(checked) {
            document.querySelectorAll('#tableBody .batch-cb').forEach(cb => {
                cb.checked = checked;
                if (checked) batchSelected.add(cb.dataset.id); else batchSelected.delete(cb.dataset.id);
            });
            advUpdateBatchToolbar();
        }

        window.advClearBatchSelection = function() {
            batchSelected.clear();
        ...
        function advUpdateBatchToolbar() {
            ...
            bar.classList.toggle('show', batchSelected.size > 0);
        }

        window.advBatchAction = async function(action) {
            if (batchSelected.size === 0) return;
            const labels = { launch: '启动', stop: '结束', delete: '删除' };
            if (!confirm(`确定批量${labels[action]}选中的 ${batchSelected.size} 个任务？`)) return;
            const d = await advFetchJson(`${ADV_API}/batch/${action}`, {
                method: 'POST', headers: getAuthHeaders(),
                body: JSON.stringify({ workflowIds: Array.from(batchSelected) })
            });
            if (d.success) {
                const r = d.data || {};
        ...
        window.advBatchExport = async function() {
            if (batchSelected.size === 0) return;
            ...
            showNotification(`已导出 ${batchSelected.size} 个任务的配置`, 'success');
        }
        ...
        window.selectDrMode = function(mode) {
        ...
        window.selectDrDbType = function(type) {
        ...
        function subRenderSelectedObjects() {
            const list = document.getElementById('subSelectedObjectsList');
            const dbs = Object.keys(subSelectedSyncObjects);
            if (dbs.length === 0) {
                list.innerHTML = '<div class="empty-selection">暂未选择任何对象</div>';
                return;
            }
            let html = '';
            dbs.forEach(db => {
                const tables = subSelectedSyncObjects[db] || [];
                html += `<div class="database-item"><div class="database-header"><span class="database-name">${db}</span></div>`;
                tables.forEach(t => {
                    html += `<div class="table-item"><span class="table-name">${t}</span></div>`;
                });
                html += '</div>';
            });
            list.innerHTML = html;
        ...
        // 关键字直接取自查询框（回车与刷新按钮共用）：空 = 不按名称/ID 过滤
        window.subscribeApplyKeywordAndRefresh = function() {
